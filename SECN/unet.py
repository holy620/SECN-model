...
class DynamicUnetWide(SequentialEx):
    ...
    def __init__(
        ...
        **kwargs
        ...
        pae_layers = set()
        if use_physical_enhancement and len(sfs_idxs) > 0:
            candidate_idxs = {0, len(sfs_idxs) // 2, len(sfs_idxs) - 1}
            pae_layers = {idx for idx in candidate_idxs if 0 <= idx < len(sfs_idxs)}
        ...
        for i, idx in enumerate(sfs_idxs):
            ...
            layers.append(unet_block)
            x = unet_block(x)
            if use_physical_enhancement and i in pae_layers:
                pae_block = PhysicalAwareEnhancement(int(x.shape[1]))
                layers.append(pae_block)
                x = pae_block(x)
