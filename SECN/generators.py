...
def gen_inference_wide(
    ...
    arch=models.resnet101, use_physical_enhancement: bool = False  # 添加参数
) -> Learner:
    ...
    learn = gen_learner_wide(
        ...
        nf_factor=nf_factor, 
        arch=arch,
        use_physical_enhancement=use_physical_enhancement  # 传递参数
    )
...
def gen_learner_wide(
    ...
    use_physical_enhancement: bool = False  # 添加此参数
) -> Learner:
...
def unet_learner_wide(
    ...
    bottle: bool = False,
    nf_factor: int = 1,
    use_physical_enhancement: bool = False,
    wd: float = None,
    loss_func = None,  # 明确提取loss_func参数
    lr: float = None,  # 添加lr参数
    **kwargs: Any
) -> Learner:
    ...
    model = to_device(
        DynamicUnetWide(
            ...
            bottle=bottle,
            nf_factor=nf_factor,
            use_physical_enhancement=use_physical_enhancement,
            **model_kwargs
        ),
        data.device,
    )
    ...
    if wd is not None:
        learner_kwargs['wd'] = wd
    if loss_func is not None:
        learner_kwargs['loss_func'] = loss_func
    learn = Learner(data, model, **learner_kwargs)
    # 完全跳过split操作，避免IndexError
    if lr is not None:
        learn.lr = lr
    apply_init(model.middle_conv[0], nn.init.kaiming_normal_)
    return learn
