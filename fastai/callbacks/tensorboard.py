...
class ModelImageSet():
    ...
    @staticmethod
    def get_list_from_model(learn:Learner, ds_type:DatasetType, batch:Tuple)->[]: # type: ignore
...
class ModelStatsTBRequest(TBWriteRequest):
    ...
    def _write_avg_norm(self, norms:[])->None: # type: ignore
    ...
    def _write_median_norm(self, norms:[])->None: # type: ignore
...
class ImageTBRequest(TBWriteRequest):
    ...
    def _write_images(self, name:str, images:[Tensor])->None: # type: ignore
    ...
    def _get_image_tensors(self)->([Tensor], [Tensor], [Tensor]): # type: ignore
